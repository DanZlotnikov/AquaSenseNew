"""ML vs GT comparison on ACQ_5_3_2026 using carp_presence_v4 ensemble (14-channel)."""
...
CKPT_DIR   = _ROOT / "checkpoints" / "carp_presence_v4"
CFG_PATH   = _ROOT / "model" / "model_config_14ch.yaml"
OUT_TXT    = DATA_DIR / "ml_vs_gt_comparison_v4.txt"
...
def add_envelope(norm12):
    """Append F_env and B_env as channels 13 and 14."""
    f_env = np.abs(norm12[:, F_IDX]).max(axis=1, keepdims=True)
    b_env = np.abs(norm12[:, B_IDX]).max(axis=1, keepdims=True)
    return np.concatenate([norm12, f_env, b_env], axis=1)
...
def infer(csv_path, models, device):
    ...
    norm  = add_envelope(normalize(raw))              # (N, 14)
...
def main():
    ...
    lines.append(f"Detection model : carp_presence_v4 ensemble ({CKPT_DIR.name})")
    lines.append(f"Normalization   : per-recording z-score, SIGMA_FLOOR={SIGMA_FLOOR}")
    lines.append(f"Extra channels  : F_env = max|z| front ring, B_env = max|z| back ring")
    lines.append(f"NMS             : threshold={THRESHOLD}, merge_gap={MERGE_GAP}")
    lines.append("GT source       : output_report.csv, is_valid==True")
    lines.append("=" * 72)
    lines.append(f"  {'File':<25}  {'GT':>5}  {'ML':>5}  {'Diff':>6}  {'Diff%':>8}")
    lines.append("-" * 72)
    ...
    lines.append(f"Files GT>0      : {sum(1 for r in rows if r[1]>0)}")
    lines.append(f"Files GT=0,ML=0 : {sum(1 for r in rows if r[1]==0 and r[2]==0)}  (clean negatives)")
    lines.append(f"Files GT=0,ML>0 : {sum(1 for r in rows if r[1]==0 and r[2]>0)}  (false positives)")
    lines.append(f"Files GT>0,ML=0 : {sum(1 for r in rows if r[1]>0 and r[2]==0)}  (missed)")
