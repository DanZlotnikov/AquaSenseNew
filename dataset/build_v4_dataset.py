"""Build train / val / test .npz files (v4 dataset — 12 acoustic channels only).

Run from the project root:
    python dataset/build_v4_dataset.py

Key difference from v3
----------------------
IMP13, IMP14, IMP15, IMP16, Distance_cm and Manual_button are excluded entirely.
Only the 12 front/back acoustic sensor channels are used as model input.
...
X shape: (N, 39, 12)  — 12 raw channels, 39 time steps at 40 Hz.

Split, stratification, oversampling, and normalisation are identical to v3.
...
class RecordingCache:
    """Lazy-loads recording CSV files and caches them as (n_rows, 12) arrays."""
...
def build_positive_samples(detections: pd.DataFrame, cache: RecordingCache) -> dict:
    ...
    Returns dict with keys: wave (N,39,12), y_presence, y_length, y_weight,
    length_class.
    """
...
def main() -> None:
    rng     = np.random.default_rng(RANDOM_SEED)
    out_dir = Path("data/v4")
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 60)
    print("Building v4 dataset  (12 acoustic channels only)")
    print("Excluded: IMP13, IMP14, IMP15, IMP16, Distance_cm, Manual_button")
    print("=" * 60)
    ...
    # Build report
    report_lines = [
        "v4 Dataset Build Report  (12 acoustic channels only)",
        "=" * 60,
        f"Window size     : {WINDOW_SIZE} samples at 40 Hz",
        f"Input channels  : {INPUT_CHANNELS}  (acoustic only — no IMP/Distance)",
        f"Presence ratio  : {PRESENCE_RATIO:.0%} positive",
        f"Split           : {TRAIN_FRAC:.0%} / {VAL_FRAC:.0%} / 10%",
        "",
        "Excluded columns: IMP13, IMP14, IMP15, IMP16, Distance_cm, Manual_button",
        "",
    ...
    report_lines += [
        ...
        "  Update training/train_config.yaml       ->  data_dir: data/v4/",
        "  Update model/model_config.yaml          ->  input_channels: 12",
        "                                              physics_dim: 0",
    ]
    ...
    print("\n" + "=" * 60)
    print("Done.  v4 dataset saved to data/v4/")
    print("=" * 60)
