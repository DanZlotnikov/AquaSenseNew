...
Each .npz split file (v2) contains:
    ...
    y_length   : (N,)         float64 — fish length in cm; 0 when fish is absent
    y_weight   : (N,)         float64 — fish weight in g;  0 when fish is absent (unused here)
"""
...
class FishDataset(Dataset):
    """Torch Dataset wrapping a single .npz split file."""
    ...
    def __init__(self, npz_path: str):
        ...
        self.length   = torch.from_numpy(data["y_length"].astype(np.float32))     # (N,)
...
def make_dataloader(
    ...
    """Return a DataLoader for the given .npz split file."""
    dataset = FishDataset(npz_path)
    return DataLoader(
        dataset,
